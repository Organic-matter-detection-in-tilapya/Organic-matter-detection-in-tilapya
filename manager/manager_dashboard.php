...
$staff_stmt = $pdo->query("SELECT user_id, full_name, email, role, assigned_pond, last_login FROM users WHERE role = 'staff' ORDER BY full_name ASC");
$staff_list = [];
while ($row = $staff_stmt->fetch()) {
    $row['status'] = ($row['last_login'] && ((time() - strtotime($row['last_login'])) / 86400) <= 7) ? 'active' : 'inactive';
    $staff_list[] = $row;
}
...
try {
    ...
    $ds = $pdo->query($dq);
    if ($ds && $ds->rowCount() > 0) {
        while ($row = $ds->fetch()) { $chart_data['daily']['labels'][]=$row['hour']; $chart_data['daily']['organic'][]=round($row['ao']??0,1); $chart_data['daily']['temperature'][]=round($row['at2']??0,1); $chart_data['daily']['ph'][]=round($row['ap']??0,1); }
    }
} catch(Exception $e) {}
if (empty($chart_data['daily']['labels'])) {
    for ($i=23;$i>=0;$i--) { $chart_data['daily']['labels'][]=date('H:00',strtotime("-$i hours")); $chart_data['daily']['organic'][]=rand(45,85); $chart_data['daily']['temperature'][]=rand(25,33); $chart_data['daily']['ph'][]=rand(65,85)/10; }
}
...
$total_ponds=$count=$safe_ponds=$warning_ponds=$critical_ponds=0;
...
if (isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        ...
        case 'send_mgr_notif':
            ...
            if (empty($msg)) { echo json_encode(['success'=>false,'message'=>'Message is required']); exit; }
            try {
                $ins = $pdo->prepare("INSERT INTO manager_notifications
                    (message, pond_name, priority, sender_id, sender_name, status, sent_at)
                    VALUES (?, ?, ?, ?, ?, 'Pending', NOW())");
                $ins->execute([$msg, $pond, $priority, $manager_id, $manager_name]);
                echo json_encode(['success'=>true,'message'=>'Notification sent to admin','id'=>$pdo->lastInsertId()]);
            } catch(Exception $e) {
        ...
        case 'get_mgr_notifs':
            try {
                $rows = $pdo->prepare("SELECT id, message, pond_name, priority, status,
                    sent_at, received_at, completed_at, admin_note
...
<style>
/* ============================================================
   BACKGROUND CHANGE: dark theme → white theme
   Only --bg-*, --txt*, --muted, --bdr variables are changed.
   All other variables, layout, and functionality are unchanged.
   ============================================================ */
:root{--bg-deep:#ffffff;--bg-panel:#f0f4f8;--bg-card:#e8eef5;--bg-elevated:#dde5ef;--bg-hover:#ccd7e5;--cyan:#00e5ff;--green:#39ff8a;--amber:#ffb800;--red:#ff3b5c;--violet:#b06cff;--teal:#00c9b1;--txt:#0d1e2e;--txt2:#2a4a6a;--muted:#5a7a9a;--bdr:rgba(0,100,160,.15);--bdr-glow:rgba(0,100,160,.4);--fd:'Syne',sans-serif;--fm:'Space Mono',monospace;--r-sm:8px;--r-md:14px;--r-lg:20px;--r-xl:28px;--nav-h:62px;--sidebar-w:260px;--bnav-h:60px;}
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
html{height:100%;scroll-behavior:smooth}
body{background:#ffffff;color:var(--txt);font-family:var(--fd);min-height:100vh;overflow-x:hidden;}
body::before{content:'';position:fixed;inset:0;z-index:0;pointer-events:none;background-image:linear-gradient(rgba(0,100,160,.06) 1px,transparent 1px),linear-gradient(90deg,rgba(0,100,160,.06) 1px,transparent 1px);background-size:44px 44px;animation:gridDrift 28s linear infinite;}
...
.sidebar{width:var(--sidebar-w);flex-shrink:0;background:rgba(240,244,248,.97);backdrop-filter:blur(20px);border-right:1px solid var(--bdr);position:fixed;top:0;left:0;bottom:0;display:flex;flex-direction:column;z-index:700;transition:transform .3s cubic-bezier(.4,0,.2,1);overflow-y:auto;overflow-x:hidden;}
...
.sidebar-title{font-size:.95rem;font-weight:800;background:linear-gradient(90deg,var(--teal),var(--txt));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1.15}
...
.topnav{display:none;background:rgba(240,244,248,.97);backdrop-filter:blur(20px);border-bottom:1px solid var(--bdr);height:var(--nav-h);padding:0 1rem;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:200;}
...
.bottom-nav{display:none;position:fixed;bottom:0;left:0;right:0;z-index:9999;background:rgba(240,244,248,.98);backdrop-filter:blur(20px);border-top:1px solid var(--bdr);grid-template-columns:repeat(5,1fr);gap:0;padding-bottom:env(safe-area-inset-bottom,0px);pointer-events:all;}
...
.kpi:hover{transform:translateY(-3px);border-color:var(--bdr-glow);box-shadow:0 0 28px rgba(0,100,160,.1)}
...
.kpi-icon{width:38px;height:38px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:.95rem;color:var(--kc,var(--teal));background:rgba(0,0,0,.06);border:1px solid currentColor;opacity:.9;margin-bottom:.7rem}
...
.btn-primary{background:var(--teal);color:#000}.btn-primary:hover{background:#00b5a0}
...
.metric-chip{text-align:center;padding:.5rem .3rem;background:rgba(0,0,0,.05);border-radius:8px;transition:.22s}
.metric-chip:hover{background:rgba(0,100,160,.07)}
...
.pond-bar{height:4px;background:rgba(0,0,0,.1);border-radius:2px;overflow:hidden}
...
.leaflet-container{background:#e8eef5!important;font-family:var(--fd)!important}
.leaflet-popup-content-wrapper{background:#f0f4f8!important;border:1px solid var(--bdr)!important;border-radius:var(--r-md)!important;box-shadow:0 8px 32px rgba(0,0,0,.12)!important;color:var(--txt)!important}
.leaflet-popup-tip{background:#f0f4f8!important}
.leaflet-control-zoom a{background:#f0f4f8!important;color:#0d1e2e!important;border-color:rgba(0,100,160,.2)!important}
.leaflet-control-attribution{background:rgba(240,244,248,.92)!important;color:var(--muted)!important;font-size:.55rem!important}
...
.alert-item{display:flex;gap:.75rem;padding:.8rem;border-radius:var(--r-md);margin-bottom:.45rem;border-left:3px solid transparent;background:rgba(0,0,0,.04);cursor:pointer;transition:.22s;animation:slideIn .3s ease both;-webkit-tap-highlight-color:transparent}
...
.rpt-preview{background:rgba(0,0,0,.04);border-radius:var(--r-lg);padding:1.2rem;border:1px solid var(--bdr)}
...
.rpt-status-row{display:flex;justify-content:space-around;padding:.6rem;background:rgba(0,0,0,.04);border-radius:8px;margin-bottom:.7rem}
...
.metric-mini{display:flex;align-items:center;gap:.5rem;padding:.5rem .65rem;background:rgba(0,0,0,.04);border-radius:8px;border:1px solid var(--bdr)}
...
.act-item{display:flex;align-items:center;gap:.7rem;padding:.6rem .4rem;border-bottom:1px solid rgba(0,0,0,.06);font-size:.8rem}
...
.mgr-notif-form textarea::placeholder{color:var(--muted)}
.mgr-notif-row{display:flex;gap:.55rem;margin-top:.65rem;flex-wrap:wrap;align-items:center}
.mgr-notif-select{background:var(--bg-card);border:1px solid var(--bdr);color:var(--txt);border-radius:var(--r-sm);padding:.45rem .75rem;font-family:var(--fd);font-size:.8rem;outline:none;-webkit-appearance:none;cursor:pointer}
.mgr-notif-select:focus{border-color:var(--teal)}
...
.modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);backdrop-filter:blur(8px);z-index:10000;align-items:center;justify-content:center;padding:1rem}
...
.meter{height:6px;background:rgba(0,0,0,.1);border-radius:3px;overflow:hidden;margin-top:.5rem}
...
@media(max-width:768px){
    ...
    .topbar{padding:.6rem .9rem}.topbar-left{gap:.5rem}
    .kpi-grid{grid-template-columns:repeat(2,1fr);gap:.7rem}
    .kpi{padding:.95rem 1rem}.kpi-val{font-size:1.5rem}
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-head">
        ...
        <div>
            ...
            <div class="sidebar-sub">Manager · Manolo Fortich</div>
        </div>
    </div>
    ...
    <div class="nav-item" onclick="showSection('mgr-notifs',this)"><i class="fas fa-paper-plane"></i> Notify Admin <span class="nav-badge" id="mgrPendingBadge" style="display:none"></span></div>
    <div class="sidebar-footer">
        <div class="sidebar-user">
            <div class="sidebar-avatar"><?php $i=''; foreach(explode(' ',$manager_name) as $n) $i.=strtoupper(substr($n,0,1)); echo $i?:'M'; ?></div>
            <div>
...
<div class="topbar">
    <div class="topbar-left">
        <div><div class="topbar-day"><?php echo $current_day; ?></div><div class="topbar-date"><?php echo $current_date; ?></div></div>
...
<div class="section-panel active" id="sec-overview">
    ...
    <div class="card">
        ...
        <div class="staff-grid">
            <?php foreach($staff_list as $s): $init=''; foreach(explode(' ',$s['full_name']) as $n) $init.=strtoupper(substr($n,0,1)); $pstatus=($ponds_data[$s['assigned_pond']]??null)['status']??'safe'; ?>
            ...
            <?php endforeach; if(empty($staff_list)): ?><div style="grid-column:1/-1;text-align:center;padding:1.5rem;color:var(--muted)">No staff assigned</div><?php endif; ?>
        </div>
    </div>
    <div class="card">
        <div class="card-head">
        ...
        <div class="pond-cards-grid">
            <?php foreach($ponds_data as $key => $pond): $sc=$pond['status']==='safe'?'var(--green)':($pond['status']==='warning'?'var(--amber)':'var(--red)'); $bc=$pond['organic_level']>80?'var(--red)':($pond['organic_level']>60?'var(--amber)':'var(--green)'); ?>
            <div class="pond-card <?php echo $pond['status']; ?>" id="pcard-<?php echo $key; ?>" onclick="showPondModal('<?php echo $key; ?>')">
                ...
                <div class="pond-bar-wrap"><div class="pond-bar-label"><span>Organic</span><span id="pb-o-<?php echo $key; ?>"><?php echo $pond['organic_level']; ?>%</span></div><div class="pond-bar"><div class="pond-bar-fill" id="pf-o-<?php echo $key; ?>" style="width:<?php echo min(100,$pond['organic_level']); ?>%;background:<?php echo $bc; ?>"></div></div></div>
    ...
    <div class="card">
        <div class="card-head"><div class="card-title"><i class="fas fa-bell"></i> Recent Alerts</div><button class="btn btn-ghost btn-sm" onclick="showSection('alerts')"><i class="fas fa-arrow-right"></i> All</button></div>
        <div style="max-height:220px;overflow-y:auto">
            ...
            <div class="alert-item <?php echo $al['type']; ?>">
                ...
                <div style="flex:1;min-width:0"><div style="display:flex;justify-content:space-between;gap:.4rem;flex-wrap:wrap"><div class="alert-pond">Pond <?php echo htmlspecialchars($al['pond_name']); ?></div><div class="alert-time"><?php echo date('h:i A',strtotime($al['created_at'])); ?></div></div><div class="alert-msg"><?php echo htmlspecialchars($al['message']); ?></div></div>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
...
<div class="section-panel" id="sec-staff">
    <div class="card">
        <div class="card-head"><div class="card-title"><i class="fas fa-users"></i> My Staff</div><span class="badge badge-info"><?php echo count($staff_list); ?> TOTAL</span></div>
        <div class="staff-grid">
            <?php foreach($staff_list as $s): $init=''; foreach(explode(' ',$s['full_name']) as $n) $init.=strtoupper(substr($n,0,1)); $pstatus=($ponds_data[$s['assigned_pond']]??null)['status']??'safe'; ?>
            <div class="staff-card" onclick="gotoMap('<?php echo $s['assigned_pond']; ?>')">
                <div class="staff-avatar"><?php echo $init?:'?'; ?></div>
                ...
                <div class="staff-foot"><span class="badge badge-<?php echo $s['status']; ?>"><span class="dot-blink"></span><?php echo strtoupper($s['status']); ?></span><div style="font-family:var(--fm);font-size:.63rem;color:var(--muted)"><i class="far fa-clock"></i> <?php echo $s['last_login']?date('M d, h:i A',strtotime($s['last_login'])):'Never logged in'; ?></div></div>
            </div>
            <?php endforeach; if(empty($staff_list)): ?><div style="grid-column:1/-1;text-align:center;padding:2rem;color:var(--muted)">No staff found</div><?php endif; ?>
        </div>
    </div>
</div>
...
<div class="section-panel" id="sec-ponds">
    <div class="card">
        <div class="card-head"><div class="card-title"><i class="fas fa-layer-group"></i> All Pond Status</div><button class="btn btn-ghost btn-sm" onclick="refreshAllPonds()"><i class="fas fa-sync-alt"></i> Refresh All</button></div>
        <div class="pond-cards-grid">
            <?php foreach($ponds_data as $key => $pond): $sc=$pond['status']==='safe'?'var(--green)':($pond['status']==='warning'?'var(--amber)':'var(--red)'); $bc=$pond['organic_level']>80?'var(--red)':($pond['organic_level']>60?'var(--amber)':'var(--green)'); ?>
...
<div class="section-panel" id="sec-charts">
    <div class="card">
        <div class="card-head">
            ...
            <div class="period-tabs"><button class="period-btn active" onclick="switchPeriod('daily',this)">DAILY</button><button class="period-btn" onclick="switchPeriod('weekly',this)">WEEKLY</button><button class="period-btn" onclick="switchPeriod('monthly',this)">MONTHLY</button></div>
        </div>
...
<div class="section-panel" id="sec-alerts">
    ...
    <?php endif; ?>
    <div class="card">
        <div class="card-head"><div class="card-title"><i class="fas fa-bell"></i> Alerts & Notifications</div><span class="badge badge-unread" id="alertBadge"><?php echo $new_alerts_count; ?> NEW</span></div>
        <div id="alertsList">
...
<div class="section-panel" id="sec-reports">
    <div class="card">
        ...
        <div class="rpt-preview" id="rptPreview">
            ...
            <div class="rpt-stats"><div class="rpt-stat"><div class="rpt-stat-val"><?php echo $total_ponds; ?></div><div class="rpt-stat-lbl">Total Ponds</div></div><div class="rpt-stat"><div class="rpt-stat-val" style="color:var(--red)"><?php echo $new_alerts_count; ?></div><div class="rpt-stat-lbl">Alerts</div></div><div class="rpt-stat"><div class="rpt-stat-val" style="color:var(--green)"><?php echo count(array_filter($staff_list,fn($s)=>$s['status']=='active')); ?></div><div class="rpt-stat-lbl">Active Staff</div></div></div>
            <div class="rpt-status-row"><div class="rpt-status-item"><div class="rpt-status-val" style="color:var(--green)"><?php echo $safe_ponds; ?></div><div class="rpt-status-lbl" style="color:var(--green)">Safe</div></div><div class="rpt-status-item"><div class="rpt-status-val" style="color:var(--amber)"><?php echo $warning_ponds; ?></div><div class="rpt-status-lbl" style="color:var(--amber)">Warning</div></div><div class="rpt-status-item"><div class="rpt-status-val" style="color:var(--red)"><?php echo $critical_ponds; ?></div><div class="rpt-status-lbl" style="color:var(--red)">Critical</div></div></div>
            <div class="metrics-mini"><div class="metric-mini"><i class="fas fa-seedling ic-organic"></i><div><div class="metric-mini-val"><?php echo $avg_organic; ?>%</div><div class="metric-mini-lbl">Avg Organic</div></div></div><div class="metric-mini"><i class="fas fa-thermometer-half ic-temp"></i><div><div class="metric-mini-val"><?php echo $avg_temp; ?>°C</div><div class="metric-mini-lbl">Avg Temp</div></div></div><div class="metric-mini"><i class="fas fa-flask ic-ph"></i><div><div class="metric-mini-val"><?php echo $avg_ph; ?></div><div class="metric-mini-lbl">Avg pH</div></div></div></div>
...
<div class="section-panel" id="sec-activities">
    <div class="card">
        ...
        <div class="act-item"><div class="act-icon <?php echo $act['type']; ?>"><i class="fas fa-<?php echo $ic; ?>"></i></div><div class="act-text"><?php echo htmlspecialchars($act['action']); ?></div><div class="act-time"><?php echo date('h:i A',strtotime($act['timestamp'])); ?></div></div>
    ...
    <div class="card">
        ...
        <div class="mgr-notif-form">
            ...
            <div class="mgr-notif-row">
                ...
                <select class="mgr-notif-select" id="mgrNotifPriority">
                    <option value="low">🟢 Low Priority</option>
                    <option value="normal" selected>🔵 Normal</option>
                    <option value="high">🟡 High Priority</option>
                    <option value="critical">🔴 Critical</option>
                </select>
                <button class="btn btn-primary" id="btnSendMgrNotif" onclick="sendMgrNotif()" style="margin-left:auto">
        ...
        <div id="mgrNotifsList">
            <div style="text-align:center;padding:1.5rem;color:var(--muted);font-family:var(--fm);font-size:.78rem">
...
const ALL_SECTIONS  = ['overview','staff','ponds','map','charts','alerts','reports','activities','mgr-notifs'];
const BNAV_SECTIONS = ['overview','ponds','map','alerts'];
let map, metricsChart, polygons={}, mapInited=false;

document.addEventListener('DOMContentLoaded',()=>{initClock();initChart();startSimulation();setTimeout(initMap,300);});

function initClock(){setInterval(()=>{const ph=new Date().toLocaleTimeString('en-US',{timeZone:'Asia/Manila',hour12:true,hour:'2-digit',minute:'2-digit',second:'2-digit'});['mainClock','topnavClock'].forEach(id=>{const e=document.getElementById(id);if(e)e.textContent=ph;});['mapTs','chartTs','footerTs'].forEach(id=>{const e=document.getElementById(id);if(e)e.textContent=ph;});},1000);}

function toggleSidebar(){const sb=document.getElementById('sidebar'),ov=document.getElementById('sidebarOverlay');const open=sb.classList.contains('open');if(open){sb.classList.remove('open');ov.classList.remove('active');}else{sb.classList.add('open');ov.classList.add('active');}}
...
function showSection(name){
    ...
    const t=document.getElementById('sec-'+name);if(t)t.classList.add('active');
    document.querySelectorAll('.nav-item').forEach(item=>{item.classList.remove('active');const oc=item.getAttribute('onclick')||'';if(oc.indexOf("'"+name+"'")!==-1)item.classList.add('active');});
    document.querySelectorAll('.bnav-item:not(.bnav-logout)').forEach(b=>b.classList.remove('active'));
    const bnEl=document.getElementById('bn-'+name);if(bnEl)bnEl.classList.add('active');
    if(name==='map'){if(map)setTimeout(()=>map.invalidateSize(),150);else if(!mapInited){mapInited=true;setTimeout(initMap,100);}}
...
function initMap(){
    ...
    Object.keys(POND_COORDS).forEach(key=>{
        ...
        const poly=L.polygon(cfg.bounds,{color,fillColor:color,fillOpacity:pond.status==='critical'?0.35:0.25,weight:2.5,dashArray:pond.status==='critical'?'8,4':null}).addTo(map);
        ...
        polygons[key]=poly;
        const lIcon=L.divIcon({className:'',html:`<div style="background:rgba(240,244,248,.92);border:1.5px solid ${color};color:${color};font-family:'Space Mono',monospace;font-size:11px;font-weight:700;padding:4px 8px;border-radius:6px;white-space:nowrap;box-shadow:0 0 12px ${color}44;">${cfg.name}</div>`,iconAnchor:[0,0]});
...
function buildPopup(key,pond,cfg,color){return `<div style="font-family:'Syne',sans-serif;color:#0d1e2e;min-width:200px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:9px;padding-bottom:7px;border-bottom:1px solid rgba(0,0,0,.1)"><span style="background:${color}22;color:${color};border:1px solid ${color}44;padding:2px 7px;border-radius:4px;font-size:10px;font-weight:700">${pond.status.toUpperCase()}</span><strong>${cfg.name}</strong></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-bottom:9px"><div style="text-align:center;background:rgba(0,0,0,.06);padding:5px;border-radius:5px"><div style="color:#39ff8a;font-weight:700;font-size:.85rem">${pond.organic_level}%</div><div style="font-size:.58rem;color:#5a7a9a">Organic</div></div><div style="text-align:center;background:rgba(0,0,0,.06);padding:5px;border-radius:5px"><div style="color:#ffb800;font-weight:700;font-size:.85rem">${pond.temperature}°C</div><div style="font-size:.58rem;color:#5a7a9a">Temp</div></div><div style="text-align:center;background:rgba(0,0,0,.06);padding:5px;border-radius:5px"><div style="color:#b06cff;font-weight:700;font-size:.85rem">${pond.ph}</div><div style="font-size:.58rem;color:#5a7a9a">pH</div></div></div><div style="font-size:.73rem;color:#5a7a9a">👤 ${pond.staff}<br>📍 ${pond.location}</div></div>`;}
...
(function(){let sx=0,sy=0;document.addEventListener('touchstart',e=>{sx=e.touches[0].clientX;sy=e.touches[0].clientY;},{passive:true});document.addEventListener('touchend',e=>{const dx=e.changedTouches[0].clientX-sx,dy=Math.abs(e.changedTouches[0].clientY-sy);if(dy>60)return;if(dx>70&&sx<30){document.getElementById('sidebar').classList.add('open');document.getElementById('sidebarOverlay').classList.add('active');}else if(dx<-70&&document.getElementById('sidebar').classList.contains('open'))closeSidebar();},{passive:true});})();

document.addEventListener('keydown',e=>{if(document.querySelector('.modal.open'))return;if(e.altKey){const m={'1':'overview','2':'staff','3':'ponds','4':'map','5':'charts','6':'alerts','7':'reports','8':'activities'};if(m[e.key]){e.preventDefault();showSection(m[e.key]);}}});

document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){Object.keys(PONDS).forEach(key=>{fetchPost('get_iot_reading',`pond_key=${key}`).then(d=>{if(d&&d.success)updatePondDisplay(key,d.organic,d.temp,d.ph,d.status,d.timestamp);}).catch(()=>{});});}});

document.addEventListener('click',function(e){const card=e.target.closest('.pond-card,.staff-card,.kpi');if(!card)return;const rect=card.getBoundingClientRect(),r=document.createElement('span'),sz=Math.max(rect.width,rect.height);r.style.cssText=`position:absolute;border-radius:50%;width:${sz}px;height:${sz}px;left:${e.clientX-rect.left-sz/2}px;top:${e.clientY-rect.top-sz/2}px;background:rgba(0,201,177,.12);transform:scale(0);animation:rippleAnim .5s ease;pointer-events:none;z-index:0;`;if(getComputedStyle(card).position==='static')card.style.position='relative';card.appendChild(r);setTimeout(()=>r.remove(),500);});

console.log('%c Organic Matter Detection in Tilapia — Manager v2.0','color:#00c9b1;font-weight:bold;font-size:13px');
console.log('%c Manolo Fortich, Bukidnon | Alt+1…8: sections | Esc: close modal','color:#5a7a9a');
</script>
</body>
</html>